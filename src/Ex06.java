...
public class Ex06 {
    ...
    public static int random(int start, int end, int... exclude) {
         ...
         */
         ...
         for(int i:exclude)
             if(number==i)
                 number=random(start,end,exclude);
         return number;
    }
    ...
    public static int[] eliminateDuplicates(int[] array) {
    	...
    	for (int i = 0; i< array.length; i++) {
    	    int j;
    	    boolean find=false;
    	    for(j =0; j<eliminate.length && i<array.length; j++) {
    	        if (array[i] == eliminate[j]) {
    	          find = true;
    	            break;
    	        }
    	    }
    ...
    public static int[] lockers(boolean[] locker) {
        ...
        // Your code goes here...
    	 int[]openLockers = new int[locker.length+1];
         int opens =0;
         int j =0;
         for (int i = 1; i < locker.length + 1; i++) {
             while (j< openLockers.length) {
                 if(openLockers[j] == 0){
                     openLockers[j] = 1;
                     opens++;
               }
         ...
         int[]lockerIndex = new int[opens];
         int i=0;
         for(int k=0; k<openLockers.length; k++)
             if(openLockers[k]==1){
                 lockerIndex[i]= k;
                 i++;
             }
